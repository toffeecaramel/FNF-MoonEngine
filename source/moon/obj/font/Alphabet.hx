...
/**
 * This code wasn't made by me (Toffee), instead, it was made by @CobaltBar
 * All credits goes to them, they did an awesome job on this and let me use it.
 * 
 * Make sure to visit their website!
 * https://cobaltbar.github.io/
 **/
	...
	function updateText(?val:String):Void
	{
		...
		for (i => text in val.replace('\r', '').split('\n'))
		{
			...
			for (ch in text.split(''))
			{
				...
				var char = alphabetGroup.recycle(FlxSprite, function():FlxSprite{
					var spr = new FlxSprite();
					spr.frames = Paths.getSparrowAtlas('UI/fonts/alphabet');
					return spr;
				});
